            ...
            self.blocks.append(
                nn.ModuleDict({
                    ...
                    "attn": nn.MultiheadAttention(
                        dim, num_heads, dropout=attn_drop_rate, batch_first=True),
...
class BRAVE(nn.Module):
        ...
        self.temporal_modules = nn.ModuleList(
            [
                LightweightTemporalTransformer(
                    dim=192, num_heads=2, num_frames=num_frames, num_layers=1,
                    mlp_ratio=mlp_ratio, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate,
                    norm_layer=norm_layer,
                ),
                LightweightTemporalTransformer(
                    dim=384, num_heads=3, num_frames=num_frames, num_layers=1,
                    mlp_ratio=mlp_ratio, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate,
                    norm_layer=norm_layer,
                ),
                LightweightTemporalTransformer(
    ...
    def _time_batch_merge_to_nchw(self, x):
        ...
        if T != self.num_frames:
            raise ValueError(f"Temporal length must be num_frames={self.num_frames}, got T={T}")
        c = self.in_chans
        if x.shape[2] == c:
            return x.reshape(B * T, c, x.shape[3], x.shape[4]).contiguous()
        if x.shape[-1] == c:
            return x.reshape(B * T, x.shape[2], x.shape[3], x.shape[4]).permute(0, 3, 1, 2).contiguous()
        raise ValueError(
            "Cannot infer layout: use [B, T, H, W, C] (channels last) "
            f"or [B, T, C, H, W] with C={c}"
        )

    def forward(self, x):
        B = x.shape[0]
        y = self._time_batch_merge_to_nchw(x)
        ...
        y = y.view(B, self.num_frames, L, C).sum(dim=1)
        y = y.transpose(1, 2)
        y = self.avgpool(y)
        y = torch.flatten(y, 1)
        y = self.head(y)
